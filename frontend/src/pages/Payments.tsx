  ...
  return (
    <div className="space-y-8 text-white min-h-screen pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {/* Left Column: Saved Cards & Mobile Wallets */}
        ...
        {/* Right Column: Wallet, Transactions & Security */}
        <div className="lg:col-span-5 space-y-6">
          ...
          <section className="bg-[#131314] border border-[#2A2A2D] rounded-2xl p-6 shadow-lg">
            ...
            <div className="bg-gradient-to-r from-[#D4A04D]/15 to-[#C8923E]/10 border border-[#D4A04D]/20 rounded-2xl p-5 text-center relative overflow-hidden">
              ...
              <button 
                ...
                className="mt-4 bg-[#D4A04D] hover:bg-[#C8923E] text-black font-extrabold uppercase py-2.5 px-5 rounded-xl text-[10px] tracking-widest transition-all cursor-pointer"
              >
                + Add Money
              </button>
            </div>
          ...
          {/* Wallet Transactions */}
          <section className="bg-[#131314] border border-[#2A2A2D] rounded-2xl p-6 shadow-lg">
            <h3 className="text-xs text-white uppercase tracking-wider font-bold mb-4 border-b border-[#2A2A2D] pb-3 flex items-center gap-2">
              <span>📜</span> Recent Transactions
            </h3>
            <div className="space-y-3 max-h-56 overflow-y-auto pr-1">
              {transactions.length === 0 ? (
                <div className="text-center py-6 text-gray-500 text-xs">No transactions yet.</div>
              ) : (
                transactions.map((tx) => {
                  const txId = tx.id || tx._id || '';
                  return (
                    <div key={txId} className="flex justify-between items-start border-b border-[#2A2A2D]/30 pb-2 text-xs last:border-0 last:pb-0">
                      <div>
                        <div className="text-white font-semibold text-[11px]">{tx.description}</div>
                        <div className="text-[#A7A7A7] text-[9px] mt-0.5">{formatDate(tx.date)}</div>
                      </div>
